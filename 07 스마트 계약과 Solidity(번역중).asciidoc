...
[intro_chapter]에서 논의한 것처럼 이더리움에는 두 가지 유형의 계정, 즉 외부 소유 계정(EOA)과 계약 계정이 있습니다. EOA는 종종 Ethereum 플랫폼 외부의 지갑 응용 프로그램과 같은 소프트웨어를 통해 사용자가 제어합니다. 반대로 계약 계정은 Ethereum Virtual Machine에서 실행되는 프로그램 코드 (일반적으로 "스마트 계약"이라고도 함)로 제어됩니다. 요컨대, EOA는 관련 코드 또는 데이터 스토리지가 없는 간단한 계정인 반면 계약 계정에는 코드 및 데이터 스토리지가 모두 있습니다. EOA는 프로토콜 외부 및 외부에서 "실제 세계"의 개인 키를 사용하여 암호화하고 서명한 트랜잭션에 의해 제어되는 반면, 계약 계정에는 개인 키가 없으므로 스마트 계약에서 규정 한 미리 정해진 방식으로 "자체를 제어"합니다. 암호. 두 가지 계정 유형 모두 이더 리움 주소로 식별됩니다. 이 장에서는 계약 계정 및 이를 관리하는 프로그램 코드에 대해 설명합니다.
...
스마트 컨트랙트라는 용어는 여러가지 다양한 것들을 설명하기 위해 수년에 걸쳐 사용되었습니다. 1990 년대에 암호 학자 Nick Szabo는 이 용어를 만들어서 당사자들이 다른 약속을 수행하는 프로토콜을 포함하여 디지털 형식으로 지정된 약속 세트(a set of promises, specified in digital form, including protocols within which the parties perform on the other promises)로 정의했습니다. 그 이후로 스마트 계약의 개념이 발전했습니다. Ethereum의 맥락에서 Ethereum 스마트 계약은 스마트하지도 않고 법적 계약도 아니라는 점에서 실제로 이 용어는 약간 잘못된 용어입니다. 이 책에서 “스마트 계약”이라는 용어는 이더리움 가상 머신과 관련하여 이더리움 네트워크 프로토콜의 일부로서 분산 이더리움 월드 컴퓨터에서 결정적으로 실행되는 불변의 컴퓨터 프로그램(immutable computer programs that run deterministically in the context of an Ethereum Virtual Machine as part of the Ethereum network protocol—i.e., on the decentralized Ethereum world computer)을 가리킵니다.
...
스마트 컨트랙트는 단순히 컴퓨터 프로그램입니다. 이 문맥에서 '계약'이라는 단어에 법적 의미는 없습니다.
...
스마트 계약 코드는 일단 배포되면 변경할 수 없습니다. 기존 소프트웨어와 달리 스마트 계약을 수정하는 유일한 방법은 새 인스턴스를 배포하는 것입니다.
...
스마트 계약의 실행 결과는 실행을 시작한 트랜잭션의 컨텍스트와 실행 시점의 이더리움 블록체인 상태를 고려할 때 스마트 계약을 실행하는 모든 사람에게 동일합니다.
...
스마트 계약은 매우 제한된 실행 컨텍스트로 작동합니다. 자신의 상태, 호출한 트랜잭션의 컨텍스트 및 가장 최근 블록에 대한 정보에 액세스 할 수 있습니다.

Decentralized world computer(분산된 세계의 컴퓨터)::
...
EVM은 모든 이더리움 노드에서 로컬 인스턴스로 실행되지만 EVM의 모든 인스턴스는 동일한 초기 상태에서 작동하고 동일한 최종 상태를 생성하므로 시스템 전체는 단일 "월드 컴퓨터"로 작동합니다.
...
스마트 계약은 일반적으로 Solidity와 같은 고급 언어로 작성됩니다. 그러나 실행하려면 EVM에서 실행되는 하위 수준 바이트 코드로 컴파일해야합니다. 일단 컴파일되면 특수 계약 작성 트랜잭션을 사용하여 Ethereum 플랫폼에 배치되며, 이는 특수 계약 작성 주소, 즉 0x0으로 전송되는 것으로 식별됩니다 ([contract_reg] 참조). 각 계약은 이더리움 주소로 식별되며, 이는 계약 생성 트랜잭션에서 생성된 계정 및 nonce의 함수(function of the originating account and nonce)로부터 나옵니다. 계약의 이더리움 주소는 거래에서 수령인으로 사용되어 계약에 자금을 보내거나 계약 기능 중 하나를 호출할 수 있습니다. EOA와 달리 새 스마트 계약을 위해 생성한 계정과 연결된 키는 없습니다. 계약 작성자는 프로토콜 수준에서 특별한 권한을 얻지 못합니다 (스마트 계약에 명시적으로 코딩할 수는 있지만). 계약 계정의 개인 키를 받지 못하고, 이는 실제로는 존재하지 않습니다. 스마트 계약 계정이 그들 자신을 소유한다고 말할 수 있습니다.
...
중요한 것은 계약이 트랜잭션에 의해 호출된 경우에만 실행된다는 것입니다. 이더리움의 모든 스마트 계약은 궁극적으로 EOA에서 시작된 거래로 인해 실행됩니다. 계약은 다른 계약을 호출할 수있는 다른 계약을 호출 할 수 있지만, 그러한 실행 체인의 첫 번째 계약은 항상 EOA의 트랜잭션에 의해 호출되었을 것입니다. 계약은 "자체적으로" 또는 "백그라운드에서" 실행되지 않습니다. 계약은 일련의 직간접적으로 계약 콜의 일부로서, 트랜잭션이 실행을 트리거 할 때까지 휴면 상태에 있습니다(Contracts effectively lie dormant until a transaction triggers execution, either directly or indirectly as part of a chain of contract calls). 스마트 컨트랙트는 어떤 의미에서든 "병렬"로 실행되지 않는다는 점에 주목할 필요가 있습니다. 이더리움 월드 컴퓨터는 단일 스레드 머신(single-threaded machine)으로 간주될 수 있습니다.
...
*트랜잭션은 호출하는 계약 수 또는 호출시 해당 계약이 수행하는 작업에 관계없이 _원자성_입니다. 모든 실행이 성공적으로 종료된 경우에만, 전역 상태 (global state)에 변경 사항이 기록되며(계약, 계정 등) 트랜잭션이 전체적으로 실행됩니다. 성공적인 종료는 프로그램이 오류없이 실행되어 실행 종료에 도달했음을 의미합니다. 오류로 인해 실행이 실패하면 트랜잭션이 실행되지 않은 것처럼 모든 영향(상태 변경)이 “복구(rolled back)”됩니다. 실패한 거래는 여전히 시도된 것으로 기록되고, 실행을 위해 gas에 소비된 이더는 원래 계정에서 공제되지만 계약 또는 계정 상태에는 다른 영향이 없습니다.*
...
앞에서 언급했듯이 계약 코드는 변경할 수 없습니다. 그러나 계약을 “삭제”하여 코드와 코드의 내부 상태(저장소)를 주소에서 제거하여 빈 계정을 남길 수 있습니다. 계약이 삭제된 후 해당 계정 주소로 전송된 트랜잭션은 더 이상 실행할 코드가 없기 때문에 코드가 실행되지 않습니다. 계약을 삭제하려면 SELFDESTRUCT(이전의 SUICIDE)라는 EVM opcode를 실행합니다. 이 작업에는 gas 환불인 "negative gas"가 필요하므로, 저장된 상태의 삭제에서 네트워크 클라이언트 리소스의 릴리스를 장려합니다(incentivizing the release of network client resources from the deletion of stored state). 이러한 방식으로 계약을 삭제해도 블록체인 자체는 변경할 수 없으므로 계약의 거래 내역(과거)을 제거하지 않습니다. 계약 작성자가 스마트 계약에 해당 기능을 갖도록 프로그래밍 한 경우에만 SELFDESTRUCT 기능을 사용할 수 있습니다. 계약 코드에 SELFDESTRUCT opcode가 없거나 액세스 할 수 없는 경우 스마트 계약을 삭제할 수 없습니다.

[[high_level_languages]]
=== Introduction to Ethereum High-Level Languages(넘어가도 될듯)
...
EVM은 x86_64와 같은 컴퓨터 코드를 실행하는 컴퓨터의 CPU와 유사한 EVM 바이트 코드라는 특수한 형식의 코드를 실행하는 가상 컴퓨터입니다. EVM의 작동과 언어를 [evm_chapter]에서 훨씬 더 자세히 살펴볼 것입니다. 이 섹션에서는 스마트 계약이 EVM에서 실행되도록 작성되는 방법을 살펴 봅니다.

스마트 컨트랙트를 바이트 코드로 직접 프로그래밍하는 것이 가능하지만, EVM 바이트 코드는 다루기가 쉽지 않으며 프로그래머가 읽고 이해하기가 매우 어렵습니다. 대신 대부분의 이더리움 개발자는 고급 언어를 사용하여 프로그램을 작성하고 컴파일러는이를 바이트 코드로 변환합니다.
...
어떤 고급 언어든 스마트 계약을 작성하도록 adapt될 수 있지만, 임의의 언어를 EVM 바이트 코드로 컴파일할 수 있도록 조정하는 것은 상당히 번거로운 작업이며 일반적으로 어느 정도 혼란을 야기합니다. 스마트 컨트랙트는 매우 제한적이고 최소한의 실행 환경(EVM)에서 작동합니다. 또한 특수한 EVM 특정 시스템 변수 및 기능 세트를 사용할 수 있어야합니다. 따라서 스마트 계약 작성에 적합한 범용 언어를 작성하는 것보다 스마트 계약 언어를 처음부터 작성하는 것이 더 쉽습니다. 결과적으로, 스마트 계약을 프로그래밍하기 위해 많은 특수 목적 언어가 등장했습니다. Ethereum에는 EVM 실행 가능 바이트 코드를 생성하는 데 필요한 컴파일러와 함께 여러 언어가 있습니다.
...
일반적으로, 프로그래밍 언어는 두 가지 광범위한 프로그래밍 패러다임으로 분류될 수 있습니다. 선언형 및 명령형, 각각 함수형 및 절차적이라고도합니다. 선언적 프로그래밍에서는 프로그램의 논리는 표현하지만 흐름은 표현하지 않는 함수를 작성합니다. 선언적 프로그래밍은 부작용이 없는 프로그램을 만드는 데 사용됩니다. 즉 함수 외부의 상태에 대한 변경이 없습니다. 선언적 프로그래밍 언어에는 Haskell 및 SQL이 포함됩니다. 대조적으로 명령형 프로그래밍은 프로그래머가 프로그램의 논리와 흐름을 결합하는 일련의 프로 시저를 작성하는 곳입니다. 명령형 프로그래밍 언어에는 C ++ 및 Java가 포함됩니다. 일부 언어는 "하이브리드"이므로 선언적 프로그래밍을 권장하지만 명령형 프로그래밍 패러다임을 표현하는 데에도 사용될 수 있습니다. 이러한 하이브리드에는 Lisp, JavaScript 및 Python이 포함됩니다. 일반적으로 명령형 언어는 선언적 패러다임을 작성하는 데 사용될 수 있지만 종종 불완전한 코드가 생성됩니다. 이에 비해 순수한 선언적 언어는 명령형 패러다임으로 작성하는 데 사용될 수 없습니다. 순전히 선언적인 언어에는 "변수"가 없습니다.
...
명령형 프로그래밍은 프로그래머가 더 일반적으로 사용하지만 예상대로 정확하게 실행되는 프로그램을 작성하는 것은 매우 어려울 수 있습니다. 프로그램의 어떤 부분이 다른 상태를 변경하는 능력은 프로그램의 실행에 대한 추론을 어렵게하고 많은 버그에 대한 기회를 제공합니다. 이에 비해 선언적 프로그래밍은 프로그램의 작동 방식을보다 쉽게 이해할 수 있도록 합니다. 부작용이 없으므로 프로그램의 모든 부분을 독립적으로 이해할 수 있습니다.

스마트 계약에서, 버그는 문자 그대로 비용이 듭니다. 결과적으로 의도하지 않은 영향없이 스마트 계약을 작성하는 것이 매우 중요합니다. 그렇게 하려면 프로그램의 예상되는 동작을 명확하게 추론할 수 있어야합니다. 따라서 선언적 언어는 범용 소프트웨어보다 스마트 계약에서 훨씬 더 큰 역할을 합니다. 그럼에도 불구하고, 스마트 계약(솔리 디티)에 가장 널리 사용되는 언어는 필수적입니다. 대부분의 인간과 마찬가지로 프로그래머도 변화에 저항합니다!
...
Solidity 프로젝트의 주요 "제품"은 Solidity 언어로 작성된 프로그램을 EVM 바이트 코드로 변환하는 Solidity 컴파일러 +solc+입니다. 이 프로젝트는 또한 이더리움 스마트 계약에 대한 중요한 ABI (Application Binary Interface) 표준을 관리합니다.이 장에서 자세히 살펴 보겠습니다. 각 버전의 Solidity 컴파일러는 특정 버전의 Solidity 언어에 해당하며 이를 컴파일합니다.
...
컴퓨터 소프트웨어에서 _application binary interface_는 두 프로그램 모듈 사이의 인터페이스입니다; 종종 운영 체제와 사용자 프로그램 사이. ABI는 _machine code_에서 데이터 구조 및 함수에 액세스하는 방법을 정의합니다. 이것은 API와 혼동되어서는 안되며,이 액세스는 _source code_와 같이 사람이 읽을 수있는 높은 수준의 형식으로 이 액세스를 정의합니다. 따라서 ABI는 머신 코드 안팎으로 데이터를 인코딩하고 디코딩하는 기본 방법입니다.

이더리움에서 ABI는 EVM에 대한 계약 요청을 인코딩하고 트랜잭션에서 데이터를 읽는 데 사용됩니다. ABI의 목적은 호출 할 수있는 계약에서 함수를 정의하고 각 함수가 인수를 승인하고 결과를 리턴하는 방법을 설명하는 것입니다.
...
보다시피, 컴파일러는 _Faucet.sol_에 의해 정의 된 두 함수를 설명하는 JSON 배열을 생성합니다. 이 JSON은 + Faucet + 계약이 배포 된 후에 액세스하려는 모든 응용 프로그램에서 사용할 수 있습니다. 월렛 또는 DApp 브라우저와 같은 애플리케이션은 ABI를 사용하여 올바른 인수 및 인수 유형으로 + Faucet +의 함수를 호출하는 트랜잭션을 구성 할 수 있습니다. 예를 들어, 지갑은 + withdraw + 함수를 호출하려면 + withdraw_amount +라는 + uint256 + 인수를 제공해야합니다. 지갑은 사용자에게 해당값을 제공하라는 메시지를 표시 한 다음 이를 암호화하고 + withdraw + 기능을 실행하는 트랜잭션을 작성합니다.
...
EVM에서 계약이 실행되면 소규모 global object 세트에 access할 수 있습니다. 여기에는 block, msg 및 tx 객체가 포함됩니다. 또한 Solidity는 사전 정의된 기능으로 여러 EVM opcode를 노출합니다. 이 섹션에서는 Solidity의 스마트 계약 내에서 액세스할 수있는 변수와 기능을 살펴 봅니다.
...
msg object는 이 계약 실행을 launch한 트랜잭션 호출(EOA에서 시작) 또는 메시지 호출(계약에서 시작)입니다. 유용한 속성이 많이 있습니다.
...
우리는 이미 이것을 사용했습니다. 이 계약 호출을 시작한 주소를 나타내며, 반드시 거래를 보낸 최초 EOA는 아닙니다. 계약이 EOA 거래에 의해 직접 호출 된 경우, 이는 거래에 서명한 주소이지만 그렇지 않으면 계약 주소가 됩니다.

+msg.value+:: The value of ether sent with this call (in wei).
이 호출과 함께 전송된 ether의 값(wei)
...
이 실행 환경의 gas supply에 남은 가스의 양(gasleft 함수로 대체됨)

+msg.data+:: The data payload of this call into our contract.
계약에 들어올 이 call의 데이터 유효 탑재량?
...
계약이 다른 계약을 호출 할 때마다 msg의 모든 속성 값이 새 발신자의 정보를 반영하도록 변경됩니다. 이에 대한 유일한 예외는 delegatecall 함수, 즉 원래 msg 컨텍스트 내에서 다른 계약/라이브러리의 코드를 실행하는 것입니다.
====
...
현재 블록에 포함된 모든 트랜잭션에서 사용할 수있는 최대 가스량
...
view로 표시된 함수는 상태를 수정하지 않을 것을 약속합니다. constant라는 용어는 향후 release에서 더 이상 사용되지 않을 view의 별칭입니다. 현재 컴파일러는 view 수정자를 적용하지 않고 경고만 생성하지만, Solidity v0.5에서는 강제 키워드가 될 것으로 예상됩니다.
...
순수한 함수는 스토리지에서 변수를 읽거나 쓰지 않는 함수입니다. 저장된 데이터를 참조하지 않고 arguments에서만 작동하고 데이터를 반환할 수 있습니다. 순수한 함수는 부작용이나 state 없는 선언적 스타일의 프로그래밍을 장려하기 위한 것입니다.
...
payable 함수는 들어오는 지불을 수락할 수 있는 기능입니다. payable로 선언되지 않은 함수는 들어오는 지불을 거부합니다. EVM의 설계 결정으로 인해 두 가지 예외가 있습니다. 폴백 함수가 payable으로 선언되지 않은 경우에도 코인베이스 지불과 SELFDESTRUCT 상속이 지불되지만 코드 실행은 이러한 지불의 일부가 아니기 때문에 말이 됩니다.
...
한 번만 사용되는 특수 기능이 있습니다. 계약이 작성되면 계약 상태를 초기화하기 위해 생성자(constructor) 함수가 있는 경우 생성자 함수도 실행합니다. 생성자는 계약 작성과 동일한 트랜잭션에서 실행됩니다. 생성자 함수는 선택 사항입니다. 수도꼭지 예에는 없습니다.
...
요약하면, 계약 수명주기는 EOA 또는 계약 계정에서 생성 트랜잭션으로 시작됩니다. 생성자가 있으면 계약 작성의 일부로 실행되어 작성중인 계약의 상태를 초기화한 다음 삭제됩니다.
...
계약 수명주기의 다른 끝은 계약 파기입니다. SELFDESTRUCT라는 특수 EVM opcode에 의해 계약이 파기됩니다. 예전에는 SUICIDE라고 불렸지만 그 단어의 부정적인 연관 때문에 그 이름은 더 이상 사용되지 않습니다. Solidity에서이 opcode는 selfdestruct라는 고급 내장 함수로 노출되며 계약 계정에 남아있는 이더 잔액을 수신하는 주소 중 하나의 인수를 취합니다. 다음과 같이 보입니다 :
...
계약을 삭제할 수있게하려면 계약에 이 명령을 명시적으로 추가해야합니다. 이는 계약을 삭제할 수있는 유일한 방법이며 기본적으로 존재하지 않습니다. 이런 식으로, 계약에 영원히 의존 할 수 있는 계약 사용자는 SELFDESTRUCT opcode가 포함되지 않은 계약을 삭제할 수 없음을 확신할 수 있습니다.
...
다음으로 계약 생성 트랜잭션의 일부로 실행되는 constructor가 + msg.sender +에서 + owner + 변수로 주소를 할당합니다. 인출 요청의 initiator를 식별하기 위해 pass : [<span class = "keep-together"> <code> withdraw </ code> </ span>] 함수에서 + msg.sender +를 사용했습니다. 그러나 constructor에서 + msg.sender +는 계약 작성을 시작한 EOA 또는 계약 주소입니다. 우리는 이것이 _because_의 경우라는 것을 알고 있습니다. 이것은 constructor 함수입니다: 계약 작성 중에 한 번만 실행됩니다.
...
*누군가 + owner + 이외의 주소에서 이 + destroy + 함수를 호출하면 실패합니다. 그러나 constructor가 + owner +에 저장된 동일한 주소로 call하면 계약 자체 소멸되어 나머지 잔액을 + owner + 주소로 보냅니다.* 우리는 소유자가 + tx.orgin +를 사용하여 계약을 파기하고자하는지 여부를 결정하기 위해 안전하지 않은 + tx.origin +를 사용하지 않았습니다.
...
계약이 오류와 함께 종료되면 둘 이상의 계약이 호출 된 경우 모든 상태 변경 (변수, 잔액 등의 변경)이 되돌려집니다. 이렇게하면 트랜잭션이 _atomic_이되므로 트랜잭션이 성공적으로 완료되거나 상태에 영향을 미치지 않으며 완전히 되돌려집니다.
...
+ assert + 및 + require + 함수는 동일한 방식으로 작동하여 조건이 거짓이면 조건을 평가하고 오류로 실행을 중지합니다. 일반적으로 + assert +는 결과가 참일 것으로 예상 될 때 사용됩니다. 즉, + assert +를 사용하여 내부 조건을 테스트합니다. 이에 비해 + require +는 입력 (예 : 함수 인수 또는 트랜잭션 필드)을 테스트할 때 사용됩니다.
...
+ require + 함수는 _gate condition_ 역할을하여 나머지 함수의 실행을 방지하고 만족하지 않으면 오류를 생성합니다.
...
이와 같은 추가 오류 검사 코드는 gas 소비를 약간 증가 시키지만 생략 된 경우보다 더 나은 오류보고 기능을 제공합니다. 계약의 예상 사용을 기반으로 gas 소비와 자세한 오류 확인간에 올바른 균형을 찾아야합니다.
