...
중요한 것은 계약이 트랜잭션에 의해 호출된 경우에만 실행된다는 것입니다. 이더리움의 모든 스마트 계약은 궁극적으로 EOA에서 시작된 거래로 인해 실행됩니다. 계약은 다른 계약을 호출할 수있는 다른 계약을 호출 할 수 있지만, 그러한 실행 체인의 첫 번째 계약은 항상 EOA의 트랜잭션에 의해 호출되었을 것입니다. 계약은 "자체적으로" 또는 "백그라운드에서" 실행되지 않습니다. 계_약은 일련의 직간접적으로 계약 콜의 일부로서, 트랜잭션이 실행을 트리거 할 때까지 휴면 상태에 있습니다(Contracts effectively lie dormant until a transaction triggers execution, either directly or indirectly as part of a chain of contract calls). 스마트 컨트랙트는 어떤 의미에서든 "병렬"로 실행되지 않는다는 점에 주목할 필요가 있습니다. 이더리움 월드 컴퓨터는 단일 스레드 머신(single-threaded machine)으로 간주될 수 있습니다.
...
다음으로 계약 작성 트랜잭션의 일부로 실행되는 생성자가 msg.sender의 주소를 소유자 변수에 지정합니다. withdrawl 요청의 개시자를 식별하기 위해 withdrawl 함수에서 msg.sender를 사용했습니다. 그러나 생성자에서 msg.sender는 계약 작성을 시작한 EOA 또는 계약 주소입니다. 우리는 이것이 생성자 함수이기 때문에 이것이 사실임을 알고 있습니다. 계약 작성 중에 한 번만 실행됩니다.
...
*누군가 owner 이외의 주소에서 이 destroy 함수를 호출하면 실패합니다. 그러나 constructor가 owner에 저장된 동일한 주소로 call하면 계약 자체 소멸되어 나머지 잔액을 owner 주소로 보냅니다.*
...
assert 및 require 함수는 동일한 방식으로 작동하여 조건이 거짓이면 조건을 평가하고 오류로 실행을 중지합니다. 일반적으로 assert는 결과가 참일 것으로 예상 될 때 사용됩니다. 즉, assert를 사용하여 내부 조건을 테스트합니다. 이에 비해 require는 함수 인수 또는 트랜잭션 필드와 같은 입력을 테스트 할 때 사용되며 이러한 조건에 대한 기대치를 설정합니다.
...
이와 같은 추가 오류 검사 코드는 gas 소비를 약간 증가 시키지만 생략된 경우보다 더 나은 오류보고 기능을 제공합니다. 계약의 예상 사용을 기반으로 gas 소비와 자세한 오류 확인간에 올바른 균형을 찾아야합니다.
