...
*계약을 삭제할 수 있게하려면 계약에 이 명령을 명시적으로 추가해야합니다. & # x2014; 이는 계약을 삭제할 수 있는 유일한 방법이며 디폴트로는 존재하지 않습니다. 이런 식으로, 계약에 영원히 의존 할 수 있는 계약 사용자는 계약이 pass를 포함하지 않으면 계약을 삭제할 수 없다는 것을 확신할 수 있습니다.* pass:[<span class = "keep-together"> <code > SELFDESTRUCT </ code> </ span>]
