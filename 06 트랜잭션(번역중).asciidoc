...
nonce: 이 주소에서 전송된 트랜잭션 수 또는 관련 코드가 있는 계정의 경우 이 계정으로 작성된 계약 작성 수와 동일한 스칼라 값입니다.
____
...
두 번의 거래를 한다고 상상해보십시오. 6 이더를 만드는 데 중요한 payment가 있으며 8 이더를 다시 지불해야합니다. 6-ether 트랜잭션이 더 중요하기 때문에 6-ether 트랜잭션을 먼저 서명하고 브로드 캐스트 한 다음 8-ether 트랜잭션을 두 번째로 서명하고 브로드 캐스트하십시오. 안타깝게도 귀하의 계정에 10 개의 에테르만 포함되어 있다는 사실을 간과했기 때문에 네트워크에서 두 거래를 모두 수락 할 수는 없습니다. 그 중 하나는 실패합니다. 가장 중요한 6 에테르를 먼저 보냈기 때문에, 하나는 통과하고 8 에테르는 거부될 것으로 예상됩니다. 그러나 이더리움과 같은 분산 시스템에서 노드는 두 가지 순서로 트랜잭션을 수신 할 수 있습니다. 특정 노드가 한 트랜잭션을 다른 트랜잭션보다 먼저 전달한다는 보장은 없습니다. 따라서 일부 노드는 6-ether 트랜잭션을 먼저 받고 다른 노드는 8-ether 트랜잭션을 먼저받습니다. nonce가 없으면 어느 것이 승인되고 어떤 것이 거부되는지는 무작위입니다. 그러나 nonce를 포함하면 보낸 첫 번째 거래의 nonce가 3이되고 8-ether 트랜잭션의 다음 nonce 값이 있습니다 (예 : 4). 따라서 0에서 3까지의 nonces가있는 트랜잭션이 수신될때까지 해당 트랜잭션은 무시됩니다.
...
이제 이더가 100 개인 계정이 있다고 가정하십시오. 당신은 온라인으로 당신이 정말로 사고 싶은 mcguffin-widget에 대한 이더 지불을 수락할 누군가를 찾습니다. 당신은 그들에게 2개의 이더를 보내고 그들은 당신에게 mcguffin-widget을 보냅니다. 그 2 이더 지불을 하기 위해, 귀하는 귀하의 계정에서 2 개의 에테르를 그들의 계좌로 보내는 거래에 서명한 다음,이를 확인하고 이더리움 네트워크에 브로드 캐스트하여 verify되고 블록 체인에 포함시킵니다. 이제 트랜잭션에 nonce 값이 없으면 두 번째로 동일한 주소로 2 ether를 보내는 두 번째 트랜잭션은 첫 번째 트랜잭션과 정확히 동일하게 보입니다. 즉, 이더리움 네트워크에서 거래를 보는 사람 (받는 사람 또는 적을 포함한 모든 사람을 의미 함)은 원래 거래를 복사하여 붙여 넣어 네트워크에 다시 보내서 모든 이더가 사라질 때까지 거래를 계속 반복해서 "replay"할 수 있음을 의미합니다. 그러나 트랜잭션 데이터에 nonce 값이 포함되어 있으면 같은 수신자의 주소로 같은 양의 에테르를 여러 번 보낼 때에도 _ 모든 단일 트랜잭션이 고유합니다 _. 따라서, 거래의 일부로 증가하는 nonce를 가짐으로써, 누군가가 귀하가 지불 한 것을 "중복"하는 것은 불가능합니다.
...
(논스란 무엇인가: https://medium.com/hexlant/%EC%9D%B4%EB%8D%94%EB%A6%AC%EC%9B%80-nonce-%EB%85%BC%EC%8A%A4-%EB%9E%80-%EB%AC%B4%EC%97%87%EC%9D%B8%EA%B0%80-60819346c803 - 논스를 이용해 이중지불 문제 방지)
...
가스는 이더리움의 연료입니다. 가스는 이더가 아닙니다. 이더에 대한 자체 환율이있는 별도의 가상 통화입니다. Ethereum은 가스를 사용하여 전 세계 수천 대의 컴퓨터에서 처리되므로 트랜잭션이 사용할 수있는 자원의 양을 제어합니다. 개방형 (Turing-complete) 계산 모델에는 서비스 거부 공격이나 실수로 리소스 소모적인 트랜잭션을 피하기 위해 어떤 형태의 계량이 필요합니다.

이더의 급격한 변화와 함께 발생할 수있는 변동성으로부터 시스템을 보호하고, 가스가 지불하는 다양한 자원의 비용 사이에서 중요하고 민감한 비율을 관리하는 방법으로 가스는 이더와 분리되어있습니다(즉, 계산, 메모리 및 스토리지).
